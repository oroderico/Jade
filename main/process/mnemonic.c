...
static wordlist_word_result_t select_wordlist_word(const bool is_mnemonic, const size_t word_index,
    ...
    const bool random_first_selection_word, gui_activity_t* enter_word_activity, gui_view_node_t* textbox,
    gui_view_node_t* backspace, gui_view_node_t* enter, gui_view_node_t* btns[], const size_t btns_len,
    ...
    *selected_word = NULL;
    ...
    while (true) {
        ...
        if (possible_words && possible_words <= NUM_WORDS_SELECT) {
            ...
            enable_relevant_chars(is_mnemonic, word, char_index, p_filter_words, num_filter_words, enter_word_activity,
                backspace, enter, btns, btns_len);
    ...
    const wordlist_purpose_t purpose, const size_t nwords, char* output, const size_t output_len)
{
    ...
    while (word_index < nwords && !done_entering_words) {
        ...
        const wordlist_word_result_t word_rslt = select_wordlist_word(is_mnemonic, word_index, wordlist_words,
            p_filter_words, num_filter_words, random_first_selection_word, enter_word_activity, textbox, backspace,
            enter, btns, btns_len, choose_word_activity, label, text_selection, &wordlist_extracted);
...
#ifndef CONFIG_DEBUG_UNATTENDED_CI
static void clear_word_number_restore_ui(digit_entry_t* digit_entry, gui_view_node_t* calc_textbox,
    gui_view_node_t* calc_text_selection, gui_view_node_t* number_text_selection)
{
    JADE_ASSERT(digit_entry);
    JADE_ASSERT(calc_textbox);
    JADE_ASSERT(calc_text_selection);
    JADE_ASSERT(number_text_selection);

    reset_digit_entry(digit_entry, "Word Number");
    gui_update_text(calc_textbox, "");
    gui_update_text(calc_text_selection, "");
    ...
    const size_t nwords, const bool advanced_mode, char* output, const size_t output_len)
{
    ...
    gui_view_node_t* calc_btns[26] = {};
    const size_t calc_btns_len = sizeof(calc_btns) / sizeof(calc_btns[0]);
    gui_view_node_t *calc_titletext = NULL, *calc_textbox = NULL, *calc_backspace = NULL, *calc_enter = NULL;
    gui_activity_t* const calc_enter_word_activity = make_enter_wordlist_word_activity(
        &calc_titletext, false, &calc_textbox, &calc_backspace, &calc_enter, calc_btns, calc_btns_len);
    JADE_ASSERT(calc_enter);
    calc_enter->is_active = false;

    gui_view_node_t* calc_text_selection = NULL;
    gui_view_node_t* calc_label = NULL;
    gui_activity_t* const calc_choose_word_activity
        = make_carousel_activity("Recover Wallet", &calc_label, &calc_text_selection);
    ...
    while (word_index < nwords) {
        char title[24];
        int ret = snprintf(title, sizeof(title), "Word %u/%u", word_index + 1, nwords);
            ...
            while (!word) {
                ...
                if (gui_activity_wait_event(
                    ...
                    if (ev_id == BTN_MNEMONIC_FINAL_WORD_EXISTING) {
                        ...
                        JADE_ASSERT(ret > 0 && ret < sizeof(enter_word_title));
                        gui_update_text(calc_titletext, enter_word_title);

                        const wordlist_word_result_t word_rslt
                            = select_wordlist_word(true, word_index, wordlist_words, final_words, num_filter_words,
                                true, calc_enter_word_activity, calc_textbox, calc_backspace, calc_enter, calc_btns,
                                calc_btns_len, calc_choose_word_activity, calc_label, calc_text_selection, &word);
        ...
        if (!word) {
            ...
            if (!run_digit_entry_loop(&digit_entry)) {
                if (word_index == 0) {
                    clear_word_number_restore_ui(
                        &digit_entry, calc_textbox, calc_text_selection, number_text_selection);
                    SENSITIVE_POP(wordlist_words);
    ...
    clear_word_number_restore_ui(&digit_entry, calc_textbox, calc_text_selection, number_text_selection);
    SENSITIVE_POP(wordlist_words);
    SENSITIVE_POP(&digit_entry);
    return nwords;
}
